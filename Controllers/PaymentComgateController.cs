    ...
    public class PaymentComgateController : BasePaymentController
    {
        ...
        public async Task<IActionResult> PDTHandler()
        {
            ...
            if (_paymentService.LoadPaymentMethodBySystemName("Payments.Comgate") is not ComgatePaymentProvider processor ||
                ...
                throw new GrandException("Comgate Standard module cannot be loaded");
        ...
        public async Task<IActionResult> IPNHandler()
        {
                ...
                switch (txn_type)
                {
                    ...
                    default:
                        #region Standard payment
                        {
                                    ...
                                    case PaymentStatus.Authorized:
                                        {
                                            ...
                                            {
                                                //not valid
                                                string errorStr = string.Format("Comgate IPN. Returned order total {0} doesn't equal order total {1}. Order# {2}.", mc_gross, order.OrderTotal * order.CurrencyRate, order.Id);
                                                //log
                                                _ = _logger.Error(errorStr);
                                                //order note
                                                await _orderService.InsertOrderNote(new OrderNote {
                            ...
                            {
                                _ = _logger.Error("Comgate IPN. Order is not found", new GrandException(sb.ToString()));
                            }
                        }
                        #endregion
                        break;
            ...
            {
                _ = _logger.Error("Comgate IPN failed.", new GrandException(strRequest));
            }
