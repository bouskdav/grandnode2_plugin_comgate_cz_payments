...
namespace Payments.Comgate.Controllers
{

    public class PaymentComgateController : BasePaymentController
    {
        ...
        private readonly IComgatePaymentService _comgatePaymentService;
        private readonly ComgatePaymentSettings _comgatePaymentSettings;

        public PaymentComgateController(
            ...
            ComgatePaymentSettings comgatePaymentSettings,
            PaymentSettings paymentSettings,
            IUserFieldService userFieldService,
            IComgatePaymentService comgatePaymentService)
        {
        ...
        //    if (_paymentService.LoadPaymentMethodBySystemName("Payments.Comgate") is not ComgatePaymentProvider processor ||
        //        !processor.IsPaymentMethodActive(_paymentSettings))
        //        throw new GrandException("Comgate  module cannot be loaded");
        ...
        //            catch (Exception exc)
        //            {
        //                _ = _logger.Error("Comgate PDT. Error getting mc_gross", exc);
        //            }
        ...
        //            var newPaymentStatus = ComgateHelper.GetPaymentStatus(payment_status, pending_reason);
        ...
        //            {
        //                string errorStr = string.Format("Comgate PDT. Returned order total {0} doesn't equal order total {1}. Order# {2}.", mc_gross, order.OrderTotal * order.CurrencyRate, order.OrderNumber);
        ...
        public async Task<IActionResult> InternalCallback(GetPaymentStatusResponse paymentStatusResponse)
        {
            ...
            string secret = _comgatePaymentSettings.ComgateSecret;

            Order order = await _comgatePaymentService.GetOrderByComgateTransactionId(paymentStatusResponse.transId);
            ...
            //order note
            await _orderService.InsertOrderNote(new OrderNote {
                Note = $"Received response from Comgate: {JsonConvert.SerializeObject(formDictionary)}",
                DisplayToCustomer = false,
                CreatedOnUtc = DateTime.UtcNow,
                OrderId = order.Id,
            });
            //order note
            await _orderService.InsertOrderNote(new OrderNote {
                Note = $"Comgate response parsed as: {JsonConvert.SerializeObject(paymentStatusResponse)}",
                DisplayToCustomer = false,
                CreatedOnUtc = DateTime.UtcNow,
                OrderId = order.Id,
            });

            var newPaymentStatus = ComgateHelper.GetPaymentStatus(paymentStatusResponse.Status);
        ...
        {
            Order order = await _comgatePaymentService.GetOrderByComgateTransactionId(transactionId);
